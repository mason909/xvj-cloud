...
// 【S-07】 房间同步 & 素材推送 // POST /api/rooms/:id/sync
app.post('/api/rooms/:id/sync', (req, res) => {
  ...
  db.query('SELECT folder_mappings, config FROM rooms WHERE id = ?', [roomId], (err, rows) => {
    ...
    // 优先从 scenes.A.folder_mappings 读，fallback 到根级 folder_mappings（兼容旧数据）
    ...
    db.query(
      ...
      (err2, devices) => {
        ...
        // 合并 Scene A 和 Scene B 的 folder_mappings（APK 同步时需要完整列表）
        ...
        devices.forEach((d) => { const mqttId = d.fingerprint || d.id;
          ...
          const syncCmd = {
            action: 'sync_room_materials',
            room_id: roomId,
            scenes: config.scenes,   // 两套场景数据（APK 渲染窗口用）
            folder_mappings: allFolderMappings, // 合并后的完整列表（APK 同步素材用）
...
app.delete('/api/rooms/:roomId/materials/:materialId', (req, res) => {
  ...
  // 查出文件名（查 materials 和 preset_materials 两个表）
  db.query('SELECT filename, name FROM materials WHERE id = ? UNION SELECT filename, name FROM preset_materials WHERE id = ?', [materialId, materialId], (err, rows) => {
    if (err) return res.status(500).json({ error: err.message });
    const filename = (rows && rows[0]?.filename) || (rows && rows[0]?.name) || null;

    db.query('SELECT id, fingerprint FROM devices WHERE room_id = ? AND authorized = 1', [roomId], (err2, devices) => {
      if (err2) return res.status(500).json({ error: err2.message });
      let sent = 0;
      devices.forEach(d => { const mqttId = d.fingerprint || d.id;
        const topic = `xvj/device/${mqttId}/command`;
        mqttClient.publish(topic, JSON.stringify({
          ...
          folder: folder,
          filename: filename   // APK 用这个定位要删除的文件
        }));
        sent++;
      });
      logAction('material_delete', 'room', { room_id: roomId, material_id: materialId, folder, filename, devices: sent });
